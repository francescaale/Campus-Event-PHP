<?php
include 'connection.php';  // Include the database connection file to establish a connection to the database
session_start();  // Start a session to maintain user login state

// Check if the session 'admin_name' is set, meaning the user is logged in as an admin
if(!isset($_SESSION['admin_name'])){
    // If the admin session is not set, redirect to the login page
    header('location:login.php');
}

// Query to fetch all events from the 'admin_event' table, ordered by date in descending order
$query = "SELECT * FROM admin_event ORDER BY date DESC";  // Fetch all events and order them by date
$result = mysqli_query($con, $query);  // Execute the query and store the result in $result (lecture 3)
?>
...
<html lang="en">  <!-- HTML language set to English -->
    <head>
        <title>Campus Events │ Your Events</title>  <!-- The title of the web page / browser tab-->
            ...
            <div class="col-md-4 mb-4">
                <div class="card shadow-sm">
                    <?php if (!empty($row['image'])) { ?>
                        <img src="<?php echo $row['image']; ?>" class="card-img-top" alt="Event Image" style="height: 200px; object-fit: cover;">
                    <?php } else { ?>
                        <img src="images/default-event.jpg" class="card-img-top" alt="Default Event Image" style="height: 200px; object-fit: cover;">
                    <?php } ?>
                    <div class="card-body text-center">
                        <h5 class="card-title"><?php echo ($row['title']); ?></h5>
                        <p class="card-text">
                            <strong>Date</strong> <br><?php echo $row['date']; ?><br>
                            <strong>Time</strong> <br><?php echo $row['time']; ?><br>
                            <strong>Location</strong> <br><?php echo ($row['location']); ?><br>
                            <strong>Price </strong> <br><?php echo $row['price'] . '£'; ?><br>
                            <strong>Age Limit</strong> <br><?php echo $row['age']; ?>+<br>
                            <strong>Description</strong> <br><?php echo $row['description']; ?><br>
                            <strong>More Info</strong> <br><?php echo $row['more_info']; ?>+
